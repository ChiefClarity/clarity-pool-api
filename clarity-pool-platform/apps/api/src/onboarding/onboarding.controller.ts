...
export class OnboardingController {
  ...
  async saveWaterChemistry(
    ...
    if (body.testStripImage) {
      analysisResult = await this.aiService.analyzeTestStrip(
        body.testStripImage,
        sessionId,
      );
    ...
    let insights = null;
    if (finalReadings) {
      insights = await this.aiService.generateWaterChemistryInsights(
        finalReadings,
        sessionId,
      );
    }
  ...
  async saveMedia(@Param('sessionId') sessionId: string, @Body() data: any) {
  ...
  async analyzePoolLocation(
    ...
    const analysis = await this.aiService.analyzePoolSatellite(
      body.address,
      sessionId,
    );
