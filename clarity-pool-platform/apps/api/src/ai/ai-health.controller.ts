...
import {
  ...
  ServiceState,
} from '../common/initialization-state.service';
  ...
  private getRecommendations(
    ...
    if (authMethod === 'api_key') {
      recommendations.push(
        '🔐 For production, use service account authentication instead of API keys',
      );
      recommendations.push(
        '📋 If using API keys, ensure they have NO HTTP referrer restrictions',
      );
    }

    if (!geminiConfigured && !claudeConfigured) {
      recommendations.push(
        '⚠️  No AI providers configured - service will not function',
      );
    } else if (!claudeConfigured) {
      recommendations.push(
        '💡 Consider configuring Claude (ANTHROPIC_API_KEY) as a fallback provider',
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        '✅ AI services are properly configured for production use',
      );
    }
