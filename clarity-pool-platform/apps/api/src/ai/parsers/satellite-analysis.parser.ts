...
import { ParsedSatelliteAnalysis } from '../types/ai-responses';
...
export class SatelliteAnalysisParser {
  ...
  private mapToAnalysisStructure(
    data: ValidatedSatelliteResponse,
    ...
    return {
      ...
      poolDimensions: dimensions
        ? {
            ...
            surfaceArea:
              this.parseNumber(dimensions.length) *
              this.parseNumber(dimensions.width),
          }
        : undefined,
      ...
      poolFeatures: {
        ...
        hasSpa:
          data.features?.includes('spa') ||
          data.spa_waterfeature?.includes('spa') ||
          false,
        hasWaterFeature:
      ...
      propertyFeatures: {
        treeCount:
          data.tree_count || this.extractTreeCount(data.surrounding_landscape),
